...
\end{minted}
%
Note the commands related to git submodules, which will correctly initialize the \texttt{libome} repository. One may also, instead of running \mintinline{bash}{init} and \mintinline{bash}{update} one may also run \mintinline{bash}{update --init}, or run \mintinline{bash}{git clone} with the option \mintinline{bash}{--recurse-submodules}, which both will clone the submodules.
...
The output is stored in an \mintinline{cpp}{std::vector<ArrayGrid>}, which the user can handle however they need; the \mintinline{cpp}{ArrayGrid} is a class that, for the purposes of accessing elements, behaves like an ordinary array; see Section~\ref{subsec:arraygrid} for more description on this class.
%
\begin{listing}
...
\begin{itemize}
  \item \mintinline{cpp}{num_grid_points}: the number of points to place in each group on the grid. See Section\ref{sec:grid} for specifically what this entails.
  ...
  \item \mintinline{cpp}{mur_muf}: the ratio $\mu_R/\mu_F$.
...
\end{itemize}
%
We also define an \mintinline{cpp}{std::vector} called \mintinline{cpp}{xtab}, which specifies a set of points which are ensured to be directly placed on the grid and kept track of, for the purpose of easily accessing the values of the distributions at those points, which is useful for benchmarking or printing out tables. This avoids interpolating to get the value of the distributions at these points. Additionally, depending on the option passed to the \mintinline{cpp}{Grid} constructor, it will also guide the distribution of points on the grid.
We then create the \mintinline{cpp}{Distribution} which contains the initial value of $\alpha_s$, the initial energy, as well as the functions defining the initial values for the distributions. The \mintinline{cpp}{LesHouchesDistribution} defines the set of initial conditions used in the standard benchmarking papers, see \comm{todo}{reference to benchmark paper}.
We use these values to create an \mintinline{cpp}{AlphaS} object, containing the information to evolve/match $\alpha_s$.
Lastly, we create a \mintinline{cpp}{DGLAPSolver} object, then call \mintinline{cpp}{.evolve()} to perform the evolution and grab the distributions.
...
The \mintinline{cpp}{ArrayGrid} class is simply a wrapper over an ordinary \mintinline{cpp}{std::vector} with a few convenience methods, though largely behaves identically. This means that one can do, for instance, list-initialization and access using the \mintinline{cpp}{operator[]} method (which returns lvalues and prvalues), like an ordinary \mintinline{cpp}{std::vector}. This is used as the datatype for the PDFs and it is what is returned to the user.

\subsubsection{Splitting Functions and OMEs}
\label{subsec:splitfuncs-and-omes}

Structurally, the splitting functions and the operator matrix elements have the following structure:
%
\begin{equation}
  P_1(x) + \frac{P_2(x)}{(1-x)^+} + P_3\delta(1-x),
\end{equation}
%
i.e. a "regular" piece, given by the $P_1$ coefficient, a "plus-distribution" piece, given by the $P_2$ coefficient (\textit{not} including the plus-distribution itself), and a "delta function" piece, given by the $P_3$ coefficient (also \textit{not} including the delta function itself).

For the purposes of convolutions with these objects, splitting functions and expressions behave identically. Therefore, we define a base class called \mintinline{cpp}{Expression} that provides methods to calculate each of the three pieces via \mintinline{cpp}{.calcX()} methods, where 'X' is ``Regular'', ``Plus'', or ``Delta,'' which are implemented by the classes \mintinline{cpp}{SplittingFunction} and \mintinline{cpp}{OpMatElem}.

Importantly, since the convolutions proceed via a Gauss-Legendre quadrature, there are a set of fixed points at which these objects are evaluated which is known before the evolution begins. Therefore, these classes contain a cache (implemented as an \mintinline{cpp}{std::map}) of the expression evaluated at those points that is filled at the start of the evolution.
This is especially helpful for higher orders where the expressions become very complex, and it saves a large amount of time.
In order to access the cached values rather than use the ``calc'' methods that each expression must implement, there are the simple \mintinline{cpp}{.regular()}, \mintinline{cpp}{.plus()}, and \mintinline{cpp}{.delta()} methods.

At $N^3LO$, we leverage the libOME library \comm{todo}{reference to libome paper}, \href{available on GitLab}{https://gitlab.com/libome/libome}, to calculate the operator matrix elements. This library provides a consistent interface, requiring only the creation of an \mintinline{cpp}{ome::rpd_distribution} which, provides a means to access the desired OME coefficients. Due to this, there is only one class, \mintinline{cpp}{OpMatElemN3LO} which accepts the corresponding OME object, at $N^3LO$.

\subsubsection{Initial Distributions}
\label{subsec:init-dists}

Initial distributions supply the values of the parton distributions at some initial energy $Q_0$, as well as the initial value of $\alpha_s$. Since this behavior is independent of the evolution, it is abstracted out into a \mintinline{cpp}{Distribution} class.
...
the values of the quark masses, the initial value of $\alpha_s$, functions for $xf_p(x)$ where $p$ is the parton, the initial value of $n_f$, the number of massless flavors at $Q_0$, as well as $Q_0$ itself.
%
We have provided two derived classes, \mintinline{cpp}{LesHouchesDistribution} and \mintinline{cpp}{LHAPDFDistribution}.
The former implements the conditions used in the benchmarks \textcolor{red}{REF}, while the latter provides an interface to supply initial conditions from an LHAPDF PDF. \footnote{Note that the latter option is only present in the compiled version of the code if the user enables the \mintinline{cmake}{WITH_LHAPDF} and, if necessary, the \mintinline{cmake}{LHAPDF_ROOT_DIR} CMake variables during configuration.}
...
\end{equation}
%
At LO, this has an exact solution, while at higher orders a fourth-order Runge Kutta method is used to determine the value of $\alpha_s$ from an initial condition.
Additionally, this class also contains the necessary information and methods to perform the matching of the coupling at the quark thresholds at NNLO and above, assuming the on-shell scheme \textcolor{red}{refer to previous section in which we define these}.
...
These values are calculated and setup only if the user calls the \mintinline{cpp}{.setVFNS()} method with the required arguments. The other option is to call the \mintinline{cpp}{.setFFNS()} with the singular value of $n_f$, which will only calculate the final value of $\alpha_s$ at $Q_f$ and do nothing else, since in the fixed-flavor number scheme, this is all that is required.

\subsubsection{The Grid}
\label{subsec:grid}

The interpolation/convolution grid is arguably the most important part of the code, as convolutions and interpolations take up the vast majority of the program's runtime. At the highest level, the \mintinline{cpp}{Grid} class supplies an underlying array of points, along with methods to perform convolutions between arrays and expressions as well as interpolations of arrays on the grid.

There are a few different prescriptions one can use to fill the grid in order to increase accuracy.
The original version of \texttt{Candia} used logarithmically spaced intervals to define the grid, extending from the minimum value provided in the array of tabulated $x$-values to 1. While this works to reduce numerical error in the low-$x$ regime in which splitting functions are singular, it fails to sufficiently fill the large-$x$ region of the grid where splitting functions are also not well-behaved.
Because of this, we also give the option (which is now the default) to fill the grid logarithmically up to only $x=0.1$, then fill it linearly from $x=0.1$ to $x=1.0$.
...
\item the ratio $\mu_r/\mu_f$ (this defaults to 1 if the user doesn't provide an argument)
\end{itemize}
%
...
Once the user calls the \mintinline{cpp}{.evolve()} method, the evolution proceeds. It will first form the special distributions the are evolved, e.g. $q_{NS}^{+}$ or $q^{(+)}$. It will then, using the initial value of $n_f$ from the distribution and the corresponding values of $\alpha_s$, perform the evolution to $Q_f$.

If the user setup the $\alpha_s$ object with the FFNS, it will do only one full set of iterations, using the values of $\alpha_s$ at $Q_0$ and $Q_f$ only. If the VFNS was setup, the evolution will be performed in several steps depending on if a quark masses is reached between $Q_0$ and $Q_f$. At each mass, the evolution only proceeds to that mass, then performs any matching (if at a sufficient perturbative order and the user has not switched matching off), then proceeds to either the next quark mass if present before $Q_f$, or just to $Q_f$ otherwise.

\subsection{Major Optimizations from \texorpdfstring{\texttt{Candia-v1}}{Candia-v1}}
\label{subsec:optims}

We now, for completeness, describe the major optimizations and general enhancements that have been made with the advance to \texttt{Candia-v2}. The first one is the flexibility offered by C++ over C when it comes to type-generic functions and interfaces. While this can be achieved in C either directly or indirectly, it often leads to un-transparent code that is hard to read and debug. C++ provides a very natural way to express, say, ``Expressions'', as in Section~\ref{subsec:splitfuncs-and-omes} with neglible overhead. While this doesn't have any significant or direct performance impacts, it makes debugging and handling the corresponding code very straightforward, leading to a faster developing process.
...
In regards to the first improvement, the choice of Gauss-Legendre quadrature leaves a fairly manageable memory footprint if we simply cache the values of the splitting functions evaluated at the Gauss-Legendre abscissae.
...
This reduces all of the piece of a calculation of a splitting function or OME to a few conditional statements to retrieve the value from the cache, and drastically speeds up the calculation of the convolutions at higher orders.

In regards to the second improvement, the non-singlet distributions that are evolved are completely decoupled, meaning they can all proceed concurrently. Assuming the user enables the \mintinline{cmake}{ENABLE_THREADING} CMake option, the code is then able to place each individual non-singlet evolution on a separate thread. The singlet sector, unfortunately, doesn't have this freedom. However, each iteration requires information only from the previous iteration, not from its own, when it comes to performing the convolutionsx. Therefore, we can split the grid up into several sections and have the convolutions performed independently on these parts, reducing the time spent on the convolutions by a considerable amount. Since essentially all remotely recent CPUs have multiple cores/threads, this is very much a viable option for performance.
