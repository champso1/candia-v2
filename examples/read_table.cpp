...
static void usage()
{
	std::cout << "USAGE: ./read <candia-file> <type>\n";
	std::cout << "    <type>: 0=all flavors independently, 1=special combos from benchmark paper" << std::endl;
	exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
	...
	if (type != 0 && type != 1) {
		log(LOG_INFO, "read_table.cpp", "Invalid type: {}", type);
		usage();
	}
