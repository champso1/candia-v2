...
namespace ome
{
  ...
  class func_plusfunc_omx
  {
    public:
      ...
      {
        using std::log;
		return target_function_(0, rest...);

		// --------------------
		// ------- NOTE -------
		// --------------------
		// Casey (11/13/2025): we don't want the log pieces or the actual 1/(1-x) piece
		// so instead of passing in log(1-x) and dividing by 1-x, I just pass in 0
		// to attempt to grab the coefficient
